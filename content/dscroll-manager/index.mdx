...
Under DScroll’s architecture, TLDs are minted as unique **NFTs** on-chain, and the underlying protocol token name is **ODude**. Instead of competing directly with ENS, DScroll goes way beyond it. It is an infrastructure that empowers *anyone*—creators, brands, communities, and DAOs—to become their own "ENS-style" identity provider using their custom minted TLD (such as `@mango` or `@eth`). 
...
*   **Current Blockchain Support**: The default deployment network is the **Base** blockchain. Support for the **BNB Chain** is currently under active development.
